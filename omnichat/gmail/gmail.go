...
// Provider implements the omnichat Provider interface for Gmail.
...
// Option configures the Gmail provider.
type Option func(*Provider)

// WithCredentialsJSON sets the OAuth credentials JSON.
func WithCredentialsJSON(creds []byte) Option {
	return func(p *Provider) {
		p.config.CredentialsJSON = creds
	}
}
...
// WithForceNewToken forces re-authentication.
func WithForceNewToken(force bool) Option {
	return func(p *Provider) {
		p.config.ForceNewToken = force
	}
}

// New creates a new Gmail provider with functional options.
func New(opts ...Option) (*Provider, error) {
	...
	if len(p.config.CredentialsJSON) == 0 {
		return nil, fmt.Errorf("gmail credentials JSON required")
	}
	if p.config.FromAddress == "" {
		p.config.FromAddress = gmailutil.UserIDMe
		p.fromAddress = gmailutil.UserIDMe
	}
	if len(p.config.Scopes) == 0 {
		p.config.Scopes = []string{gmailutil.GmailSendScope}
	}
	if p.config.TokenFile == "" {
		p.config.TokenFile = "~/.omnichat/gmail_token.json"
	}

	return p, nil
}

// NewWithConfig creates a new Gmail provider with a config struct.
func NewWithConfig(config Config) (*Provider, error) {
	...
	if config.TokenFile == "" {
		config.TokenFile = "~/.omnichat/gmail_token.json"
	}
